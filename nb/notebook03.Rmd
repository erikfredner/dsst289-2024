...
For practice, we will work again with the largest cities data sets:
...
We will also work with a second data set of U.S. city populations over time.
The code below, which we will learn about next time (note the `%in%` operator), will select three cities of data (Boston, Chicago, and LA) for us to work with:
...
In this section we will use the `cities` data set. In the code below, produce a
...
cities in the data set that you tend to expect to find on a map?
**Answer**:

## Group Aesthetic

In this section, we will use the `us_three` data set. In the code below we are
...
Now, return again to the `cities` data set. Produce a scatter plot with
...
With the `cities` data set, produce a scatter plot with city area (`city_area`)
...
or city. Information about these scores are included in the `cities` data set and
...
In the code block below, produce a scatter plot of the `cities` data set with
...
In the code below repeat the scatter plot, but add a manual

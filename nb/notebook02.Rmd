...
After starting a new session, run the following code chunk to set your working directory.

```{r, include=FALSE, message=FALSE}
if (interactive()) {
  # 1. Choose this .Rmd notebook in your DSST289/nb folder:
  this_notebook <- file.choose()
  ...
  # 3. Confirm that the working directory is set correctly:
  if (!file.exists("../funs/funs.R")) {
    stop("The working directory does not seem to be set correctly. Make sure that your notebook is located in the correct directory (`DSST289/nb`), and try again.")
  } else {
    message("The working directory appears to be set correctly.")
  }
}
```


Then, run this chunk to load the libraries and data that we will be working with today.

```{r, include=FALSE, message=FALSE}
# install the missing packages from last time:
install.packages(
  pkgs = c(

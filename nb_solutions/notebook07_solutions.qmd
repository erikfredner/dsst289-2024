...
movies |>
  # filter to retain only numeric values:
  filter(budget > 0 & gross_worldwide > 0) |>
  ggplot(aes(budget, gross_worldwide)) +
  geom_point(aes(color = year, size = contributor_rating)) +
...
```{r}
movies <- movies |>
  mutate(total_minutes = (runtime_hrs * 60) + runtime_min)

movies |>
  select(title, total_minutes) |> 
  # we only want one row per title:
  distinct()
```
...
```{r}
movies |>
  select(title, budget, total_minutes, year) |>
  # ensure that we only have one point per title:
  distinct() |> 
  ggplot(aes(budget, total_minutes)) +
...
All of the IMDB ratings *should* be the same, so we could use the `first()`
...
taking the `first()`.] Alternatively, we can also use `mean()`; the result should be the same if the data is entered properly.
...
movies |>
  group_by(title) |>
  summarize(
    mean_class_rating = mean(contributor_rating, na.rm = TRUE),
    # IMDB ratings should all be the same, but this works:
    imdb_rating = mean(imdb_rating, na.rm = TRUE)
  ) |>
  # filter out any missing or zero IMDB ratings to avoid dividing by zero:
  filter(!is.na(imdb_rating) & imdb_rating != 0) |>
  # calculate ratio:
  mutate(rating_ratio = mean_class_rating / imdb_rating) |>
  ggplot(aes(x = mean_class_rating, y = imdb_rating, size = rating_ratio)) +
  geom_point() +
  geom_text_repel(aes(label = title))
```
...
movies |>
  ...
  scale_color_viridis_b() +
  labs(
    x = "Budget (USD)", y = "Gross Worldwide (USD)",
    color = "Release Year", size = "Contributor Rating"
  )
```
...
movies |>
  ...
  labs(
    x = "Budget (USD)", y = "Gross Worldwide (USD)",
    color = "Release Year", size = "Contributor Rating",
    title = "DSST289 Movie Data", subtitle = "Home-grown organic data",
...
movies |>
  ggplot(aes(budget, gross_worldwide)) +
  geom_point(aes(color = year, size = contributor_rating)) +
  # Note the addition of the `family` argument here:
  geom_text_repel(aes(label = title), family = "Times New Roman") +
  scale_x_log10() +
  scale_y_log10() +
  scale_color_viridis_b() +
  labs(

...
```{r, question-02b}
birds |> 
  count(year = year(date), month = month(date, label = TRUE)) |>
  ggplot(aes(month, n, fill = as_factor(year))) +
  geom_col(position = "stack") +
...
birds |>
  ...
  summarize(
    sightings = n(),
    paris_sightings = sum(departement == "75"),
    paris_proportion = paris_sightings / sightings
  ) |>
  arrange(desc(paris_proportion))
```
...
birds |>
  ...
  inner_join(species, by = "scientific_name") |>
  group_by(species_common) |>
  summarize(n_departement = n_distinct(departement)) |>
...
```{r, question-09}
france |>
  inner_join(bird_count, by = "departement") |>
  st_transform(27561) |>
  ggplot() +
  geom_sf(aes(fill = n)) +
  scale_fill_distiller(

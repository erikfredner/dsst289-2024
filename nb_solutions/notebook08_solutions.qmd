...
**Answer**:
Music majors have less variability in estimated lifetime earnings than either
Psychology or Art History majors. Most music majors have more similar and lower earnings than Psychology or Art History majors.
Psychology and Art History majors appear to have similar modal earnings. However, Art History majors have higher variability in lifetime earnings than Psychology majors, with a long tail of higher earners.
...
There are multiple ways to do this task. Try to do it using `mutate` and `if_else` to construct a new color variable in your dataset. Then, use `scale_color_identity()` to map the selected colors to the points.
...
majors |>
  ...
  arrange(earnings) |>
  mutate(major = fct_inorder(major),
         color = if_else(percentile == 25, "olivedrab", "navy")) |>
  ggplot(aes(earnings, major, color = color)) +
  geom_point() +
...
majors |>
  ...
  ggplot(aes(earnings, fct_reorder(major, earnings), color = color)) +
  geom_point() +
  scale_color_identity()
```
...
majors |>
  ...
  arrange(earnings) |>
  mutate(major = fct_inorder(major),
         color = if_else(percentile == 10, "olivedrab", "navy")) |>
  ggplot(aes(earnings, major)) +
...
majors |>
  ...
  group_by(major) |>
  summarize(percentile = min(percentile)) |>
...
**Answer**: While some majors do make more money that others on average, there is a considerable range within the majors. Particularly if we ignore the small, specialized engineering majors, there is considerable overlap in the distribution of incomes. It seems that it would be better to be a well-paid geography major than a typical accounting major. So, if you love and are good at geography, you should follow that path even if you are primarily motivated by money.

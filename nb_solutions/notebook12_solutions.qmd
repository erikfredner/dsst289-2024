...
penguins |>
  ...
  ggplot(aes(avg_mass, avg_bill_length)) +
  geom_point(aes(color = scientific_name, shape = sex), size = 5) +
  scale_color_viridis_d()
```
...
penguins |>
  ...
  pivot_wider(
    names_from = sex,
    values_from = c(avg_mass, avg_bill_length)
  ) |>
  left_join(latin_names, by = c("species" = "common_name")) |>
  ggplot(
    aes(
      avg_mass_female,
      avg_bill_length_female,
  ...
  geom_segment(
    aes(
      xend = avg_mass_male,
      yend = avg_bill_length_male
    ),
    arrow = arrow(length = unit(0.02, "npc"))
  ) +
  geom_label_repel(aes(label = scientific_name)) +
  # disable legend since the arrows are labeled:
  guides(color = "none") +
  scale_color_viridis_d()
```
...
penguins |>
  ...
  scale_color_viridis_d() +
  labs(
    title = "Penguin Size Differences by Species and Sex",
    subtitle = "Arrows start from the female averages and point towards male averages",
...
food |>
  ...
  filter(kcal < 6000) |>
  ggplot(aes(kcal)) +
  geom_density(alpha = 0.5, fill = "olivedrab")
```
...
food |>
  ...
  labs(
    x = "Calories per day",
    y = "Density",
    title = "Calories per day by age group and gender",
    subtitle = "Density plot of calories consumed by individuals in the US in ten-year bins"
  )
```
...
food |>
  ...
  summarize(kcal = sum(kcal) / first(total_people)) |>
  ggplot(aes(x = factor(time), y = meal_name, fill = kcal)) +
  geom_tile() +
  scale_fill_viridis_c() +
  labs(
    title = "Average calorie intake by time and meal",
    x = "Hour of day",
    y = "Meal",

...
`TRUE` condition will onlybe reached if none of the other conditions are met.
...
is specifically for pairing a condition with the corresponding output value.
...
Here is an example of adding the average calories of each food
...
There is also a `min` function that can be used in the same way.

## `ungroup`ing data

With the group mean above, you could perform subsequent calculations.
For example, if you wanted to calculate the ratio of an individual food's
calories to the average calories of its food group, you could do the following:
...
If you want to sort the entire data set by the ratio, you will first need to `ungroup`
the data, so that the `arrange` operation is not applied to the groups:

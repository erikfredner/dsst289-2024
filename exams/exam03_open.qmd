...
Times in `storms` are all given in the "America/New_York" timezone, so you should force this timezone in `time`.
...
Plot the data above with the earliest time at which each storm reached its maximum wind speed on the x-axis and the maximum wind speed on the y-axis. Use a points layer, then label each point with the name of the hurricane, and repel the labels away from the points. Color the points by the predicted gender of the storm's name. Use the following color scale:
